...
void LeftPanel::renderEnvironmentTab() {
    if (gViewport) {
        ...
        if (ImGui::BeginCombo("Environment Preset",
        ...
        if (ImGui::Button("Import .fvdenv...")) {
            auto file = pfd::open_file("Import Environment", "skybox",
                                       {"FVD Environment", "*.fvdenv", "All Files", "*"})
                            .result();
            if (!file.empty())
                gViewport->loadEnvironmentPreset(file[0]);
        }
        ImGui::SameLine();
        if (ImGui::Button("Export .fvdenv...")) {
            std::string suggested = gViewport->suggestedEnvironmentPresetPath();
            std::string file = pfd::save_file("Export Environment", suggested,
                                              {"FVD Environment", "*.fvdenv", "All Files", "*"})
                                   .result();
            if (!file.empty()) {
                std::filesystem::path outputPath(file);
                if (lowerAscii(outputPath.extension().string()) != ".fvdenv")
                    file += ".fvdenv";
                gViewport->saveEnvironmentPreset(file);
            }
        }
    }
    ...
    if (ImGui::TreeNodeEx("Skybox", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (!gloParent->skyboxAvailable) {
            ...
            ImGui::TableNextColumn();
            ImGui::TableNextColumn();
            ImGui::TextDisabled("Add nx/ny/nz/px/py/pz or cubemap_0..5 images to a folder inside skybox/.");
        }
        END_PROP_TABLE()
        ImGui::TreePop();
    }
